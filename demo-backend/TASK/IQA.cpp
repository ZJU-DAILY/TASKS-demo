...
string stringFileName, outName, pathName;
Trie *trie;
vector<string> recs;
...
int prefixED;
void QueryTest(vector<int> &resIds) {
...
void QueryPath(vector<int> &resIds) {
    srand((int)time(0));
    path.clear();
...
int main(int argc, char **argv) {
    ...
    // rebuild trie
    stringFileName = "../datasets/" + cityName + "/" + cityName + ".string";
    ...
    while (true) {
        ...
        if(queryType == 1) {
            ...
            // cin >> input;
            input = queryVertex_inputs[info_item_num-1];
        ...
        if(resIds.size() == 0) {
            ...
            resInfo = to_string(resIds[0]);
            for(int i=1; i<resIds.size(); ++i) {
                resInfo += ("," + to_string(resIds[i]));
            }
        }
